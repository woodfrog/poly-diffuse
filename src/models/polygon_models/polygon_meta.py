...
class PolyMetaModel(nn.Module):
    ...
    forward process.
    """
    def __init__(self, input_dim, embed_dim, num_poly=20, num_vert=40):
        ...
        self.input_mlp = MLP(input_dim*2+embed_dim, embed_dim, embed_dim, 2)
    ...
    def forward(self, polys, mask):
        ...
        vert_encodings = torch.cat([vert_encodings, vertex_id_encodings], dim=-1)
        vert_encodings = rearrange(vert_encodings, 'b n1 n2 c -> b (n1 n2) c')    
        vert_encodings = self.input_mlp(vert_encodings)

        global_token = repeat(self.global_token_embedding.weight[0], 'c -> b n1 n2 c', b=bs, n1=self.num_poly, n2=1)
        vert_encodings = rearrange(vert_encodings, 'b (n1 n2) c -> b n1 n2 c', n1=self.num_poly)
        ...
        vert_encodings = self.encoder(vert_encodings, key_padding_mask=attn_mask)
