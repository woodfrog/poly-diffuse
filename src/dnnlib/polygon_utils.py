...
def process_polygons(polygons):
    ...
    for eff_poly_id in eff_poly_ids:
        ...
        # Skip too small polygons (likely are noisy outputs)
        coords_std = poly_coords.std(dim=0).mean()
        if coords_std < 0.02:
            continue
        # Debugging use: check the effecive vertex ids
        #if iter_i == 5: 
        #    xx = torch.where(pred_scores[eff_poly_id]> 0.5)
        #    print(eff_poly_id, xx, pred_scores[eff_poly_id][xx])
...
def visualize_inter_results(results, sample_idx, base_dir):
    images = []
    for idx, result in enumerate(results):
        inter_poly = th.clip(result[0], -1, 1)
        inter_polygons, inter_poly_ids = process_polygons(inter_poly.detach())
