...
def format_res_gt_by_classes(result_path,
    ...
    if os.path.exists(formatting_file):
        formatted_results = mmcv.load(formatting_file)
        print('Found existing formatted file on {}, directly loaded.'.format(formatting_file))
        cls_gens, cls_gts = formatted_results
        return cls_gens, cls_gts
...
def eval_map(gen_results,
    ...
    for i, clsname in enumerate(cls_names):
        ...
        tpfp_fn = partial(tpfp_fn, threshold=threshold, 
                          threshold_angle=threshold_angle, metric=metric,
                          consider_angle=consider_angle)
        args = []
        # compute tp and fp for each image with multiple processes

        repeat_cls_name = [clsname,] * len(cls_gen)

        tpfp = pool.starmap(
            tpfp_fn,
            zip(cls_gen, cls_gt, repeat_cls_name, *args))
        ## import pdb;pdb.set_trace()
        #tpfp = map(tpfp_fn, cls_gen, cls_gt, repeat_cls_name, *args)
