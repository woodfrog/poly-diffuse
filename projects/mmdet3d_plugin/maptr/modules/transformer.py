...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        # Caching status for the bev embedding (for polydiffuse inference)
        self.cached = False
        self.cached_bev_embed = None
    ...
    def init_layers(self):
        ...
        self.reference_points = nn.Linear(self.embed_dims+self.timestep_dim, 2)
    ...
    def init_weights(self):
        ...
        constant_init(self.reference_points, 0)
    ...
    def forward(self,
        ...
        if not use_cached_feat:
            ...
            if cache_image_feat:
                self.set_cache(bev_embed)
        else:
            bev_embed = self.cached_bev_embed

        bs = mlvl_feats[0].size(0)

        # the input x_t coords as the init ref directly
        reference_points = vert_coords.clip(0, 1)

        # NOTE: optionally use linear layer to generate the init ref points
        repeat_t_emb = repeat(t_emb, 'b n2 -> b n1 n2', n1=vert_encodings.shape[1])
        rf_encodings = torch.cat([vert_encodings, repeat_t_emb], dim=-1)
        reference_points_offset = self.reference_points(rf_encodings).sigmoid()
        ...
        query = vert_encodings.permute(1, 0, 2)
        query_pos = vert_query_embed.permute(1, 0, 2)
        ...
        inter_states, inter_references = self.decoder(
            ...
            query_key_padding_mask=poly_mask,
            **kwargs)
    ...
    def set_cache(self, bev_embed):
        self.cached = True
        self.cached_bev_embed = bev_embed
