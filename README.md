...
#### Data
Since we follow the RGB input setting as in the original [MapTR paper](https://arxiv.org/abs/2208.14437), we only need a subset of the nuScenes dataset (i.e., RGB captures from six surrounding cameras, map annotations). To simplify the data preparation process and save the disk space, we provide a zipped file for all necessary data (~40GB) via this [Dropbox folder link](https://www.dropbox.com/scl/fo/4702vswur8rw1yapnltbf/AFRI5dnkHf_MBgWx0zbzdjE?rlkey=qk8q4rlsnolrvzf7azlhkan8c&st=867hfdkb&dl=0). Please download it into ```./data```, run `cat nuscenes_maptr_processed.zip.part* > nuscenes_maptr_processed.zip` to merge the small splits, and unzip. Or you can also follow the [guidelines provided by MapTR](https://github.com/hustvl/MapTR/blob/main/docs/prepare_dataset.md) to download the entire dataset and run the preprocessing. 
